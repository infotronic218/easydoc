...
public class CasierController {
	...
	public String demandeConfirmation(Model model, Document document, 
			@RequestParam(name="idVille")Long idVille,BindingResult result) throws IOException {
		if(result.hasErrors() || document==null || document.getFile()==null || idVille==null) {
			model.addAttribute("current","SERVICES");
		...
		session.setAttribute("ville", repVille.getOne(idVille));
		model.addAttribute("current","SERVICES");
		model.addAttribute("etape",3);
		model.addAttribute("countries", codeManager.getContries());
		return "casier/demande";
